...
Hook::Hook(FARPROC originalAddress, FARPROC hookAddress, bool allowOriginalCodeUsage)
{
	...
	memcpy((void*)(trampoline + 2), &hookAddress, 8); //Filling the hooked function's address into the trampoline's code
	...
	oldProtec = NULL;
	/*
	if (allowOriginalCodeUsage) //Not useful, just bugging everywhere when set to true :'(
		allowOrigExec(); //That's shit, don't look at it
	//*/
}

void Hook::allowOrigExec()
{
	VirtualProtect(originalCode, TRAMPOLINE_SIZE, PAGE_EXECUTE_READ, &oldProtec); //That's shit, don't look at it
}

void Hook::disallowOrigExec()
{
	VirtualProtect(originalCode, TRAMPOLINE_SIZE, oldProtec, NULL); //That's shit, don't look at it
}
...
{
	for (int i = 0; i < length; i++) //For each byte in the sequence
	{
		if (par1[i] != par2[i])
